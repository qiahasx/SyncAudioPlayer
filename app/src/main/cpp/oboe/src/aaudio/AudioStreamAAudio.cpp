 ...
 */
    AudioStreamAAudio::AudioStreamAAudio(const AudioStreamBuilder &builder)
            : AudioStream(builder), mAAudioStream(nullptr) {
...
// That other thread can safely stop, close and delete the stream.
    void AudioStreamAAudio::internalErrorCallback(
        ...
        if (oboeStream->wasErrorCallbackCalled()) { // block extra error callbacks
            ...
            LOGW("%s() stream already closed or closing",
                 __func__); // might happen if there are bugs
    ...
    Result AudioStreamAAudio::open() {
        ...
        if (mLibLoader->builder_setChannelMask != nullptr &&
        ...
        mLibLoader->builder_setDirection(aaudioBuilder,
                                         static_cast<aaudio_direction_t>(mDirection));
        ...
        if (mLibLoader->builder_setInputPreset != nullptr) {
            ...
            if (getSdkVersion() <= __ANDROID_API_P__ &&
                inputPreset == InputPreset::VoicePerformance) {
        ...
        if (mLibLoader->builder_setAllowedCapturePolicy != nullptr &&
            mDirection == oboe::Direction::Output) {
        ...
        if (mLibLoader->builder_setPrivacySensitive != nullptr &&
            mDirection == oboe::Direction::Input
        ...
        if (isDataCallbackSpecified()) {
            mLibLoader->builder_setDataCallback(aaudioBuilder, oboe_aaudio_data_callback_proc,
                                                this);
        ...
        if (mLibLoader->stream_getContentType != nullptr) {
            mContentType = static_cast<ContentType>(mLibLoader->stream_getContentType(
                    mAAudioStream));
        }
        if (mLibLoader->stream_getInputPreset != nullptr) {
            mInputPreset = static_cast<InputPreset>(mLibLoader->stream_getInputPreset(
                    mAAudioStream));
        }
        ...
        // This was added in Q so we have to check for the function pointer.
        if (mLibLoader->stream_getAllowedCapturePolicy != nullptr &&
            mDirection == oboe::Direction::Output) {
            mAllowedCapturePolicy = static_cast<AllowedCapturePolicy>(mLibLoader->stream_getAllowedCapturePolicy(
                    mAAudioStream));
        } else {
            mAllowedCapturePolicy = AllowedCapturePolicy::Unspecified;
        }

        if (mLibLoader->stream_isPrivacySensitive != nullptr &&
            mDirection == oboe::Direction::Input) {
        ...
        if (mLibLoader->stream_getChannelMask != nullptr) {
            mChannelMask = static_cast<ChannelMask>(mLibLoader->stream_getChannelMask(
                    mAAudioStream));
        }
        ...
        if (mLibLoader->stream_getHardwareFormat != nullptr) {
            mHardwareFormat = static_cast<AudioFormat>(mLibLoader->stream_getHardwareFormat(
                    mAAudioStream));
        }
    ...
    DataCallbackResult AudioStreamAAudio::callOnAudioReady(AAudioStream * /*stream*/,
        ...
        if (result == DataCallbackResult::Continue) {
            ...
            if (result == DataCallbackResult::Stop) {
                ...
                LOGE("Oboe callback returned unexpected value. Error: %d",
                     static_cast<int>(result));
            }
...
#define DEBUG_CLOSE_RACE 0
    ...
    ResultWithValue<int32_t> AudioStreamAAudio::getXRunCount() {
        ...
        if (stream != nullptr) {
            return ResultWithValue<int32_t>::createBasedOnSign(
                    mLibLoader->stream_getXRunCount(stream));
        } else {
    ...
    ResultWithValue<double> AudioStreamAAudio::calculateLatencyMillis() {
        ...
        double latencyNanos = static_cast<double>(isOutput
                                                  ? (appFrameHardwareTime -
                                                     appFrameAppTime) // hardware is later
                                                  : (appFrameAppTime -
                                                     appFrameHardwareTime)); // hardware is earlier

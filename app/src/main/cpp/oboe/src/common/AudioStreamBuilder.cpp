...
#define OBOE_ENABLE_AAUDIO 1
    ...
    bool AudioStreamBuilder::isCompatible(AudioStreamBase &other) {
        return (getSampleRate() == oboe::Unspecified || getSampleRate() == other.getSampleRate())
               ...
               && (getFramesPerDataCallback() == oboe::Unspecified ||
                   getFramesPerDataCallback() == other.getFramesPerDataCallback())
               && (getChannelCount() == oboe::Unspecified ||
    ...
    Result AudioStreamBuilder::openStreamInternal(AudioStream **streamPP) {
        ...
        *streamPP = nullptr;
        ...
        bool conversionNeeded = QuirksManager::getInstance().isConversionNeeded(*this,
                                                                                childBuilder);
        ...
        if (wasMMapTemporarilyDisabled) {
            AAudioExtensions::getInstance().setMMapEnabled(
                    wasMMapOriginallyEnabled); // restore original
        }
        if (result == Result::OK) {
            ...
            if (streamP->getAudioApi() == AudioApi::AAudio) {
                ...
                if (streamP->getDirection() == Direction::Input) {
                    ...
                    optimalBufferSize = streamP->getBufferCapacityInFrames();
                           ...
                           && streamP->getDirection() ==
                              Direction::Output) { // Output check is redundant.

...
static MultiChannelResampler::Quality
convertOboeSRQualityToMCR(SampleRateConversionQuality quality) {
    switch (quality) {
    ...
    if ((isDataCallbackSpecified && isOutput)
        ...
        switch (sourceFormat) {
            ...
            case AudioFormat::I32:
                ...
                break;
            default:
                LOGE("%s() Unsupported source caller format = %d", __func__,
        ...
        switch (sourceFormat) {
            ...
            case AudioFormat::I32:
                ...
                break;
            default:
                LOGE("%s() Unsupported source format = %d", __func__,
                     static_cast<int>(sourceFormat));
                return Result::ErrorIllegalArgument;
        }
        if (isInput) {
...
int32_t DataConversionFlowGraph::onProcessFixedBlock(uint8_t *buffer, int32_t numBytes) {
    ...
    mCallbackResult = mFilterStream->getDataCallback()->onAudioReady(mFilterStream, buffer,
                                                                     numFrames);
